...
OSS does not consume `cluster_config` at runtime, but schema validation may still report warnings/errors for invalid fields.
</Warning>
...
| `service_name` | string | Service identifier used by discovery backends |
...
| `mdns_service` | string | mDNS service name |
