...
## Breaking Change 9: Migrate provider deployment mappings to `aliases`
...
<Note>
For Bedrock, legacy `bedrock_key_config.deployments` may still be accepted for backward compatibility in some setups, but `aliases` is the recommended and forward-compatible format.
</Note>
